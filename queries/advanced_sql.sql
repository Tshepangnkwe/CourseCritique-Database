...
SELECT 
    ...
    -- Use MAX to ensure single value per group
    MAX(
        CASE
            WHEN REGEXP_LIKE(r.content, 'great|excellent|amazing', 'i') THEN 'Positive'
            WHEN REGEXP_LIKE(r.content, 'poor|bad|terrible', 'i') THEN 'Negative'
            ELSE 'Neutral'
        END
...
SELECT 
    ...
    (SELECT COUNT(*) FROM Reviews r 
     ...
     AND REGEXP_LIKE(r.content, 'great|excellent', 'i')) AS positive_keyword_count,
    (SELECT TO_CHAR(MAX(r2.review_date), 'YYYY') FROM Reviews r2
...
ORDER BY TO_NUMBER(TO_CHAR(AVG(cs.average_rating), '0.0')) DESC;
...
SELECT 
    ...
    COUNT(DISTINCT 
        CASE 
            WHEN r.is_anonymous = 1 THEN r.id -- use r.id as a proxy for uniqueness when anonymised
            ELSE u.id
        END
    ) AS unique_reviewers
FROM Instructors i
...
SELECT 
    ...
    ROUND(100 * SUM(CASE 
        WHEN REGEXP_LIKE(r.content, 'great|excellent', 'i') THEN 1 ELSE 0 
    END) / COUNT(r.id), 0) || '%' AS positive_pct,
    ROUND(
        NVL(
            (AVG(CASE 
                WHEN r.review_date >= ADD_MONTHS(SYSDATE, -3) THEN r.rating 
                END) -
             AVG(CASE 
                WHEN r.review_date BETWEEN ADD_MONTHS(SYSDATE, -6) AND ADD_MONTHS(SYSDATE, -3) 
                THEN r.rating 
                END)
            ), 0
        ), 2
